...
namespace Placement
{
    class Program
    {
        ...
        {       
            ...
            for (int I = 0; I < 150; I++)
            {
                Container container = new Container(Containers.Count, 10000, ContainerType.Cooled);
                Containers.Add(container);
            }
            for (int I = 0; I < 7; I++)
            {
                ...
                if (boat.Grids != null)
                {
                    ...
                    foreach (var gridHeight in gridsGroupedByHeight)
                    {
                        ...
                        foreach (var gridDepth in gridsGroupedByDepth)
                        {
                            ...
                            var sorted = gridDepth.OrderBy(grid => grid.X).ThenBy(grid => grid.Y);
                            foreach (var grid in sorted)
                            {
                                //Console.WriteLine("Container: " + grid.Container.Id + " - (Width: " + grid.X + ", Depth: " + grid.Y + ") - Type: " + grid.Container.Type );
                                Console.WriteLine("\t\t Container: " + grid.Container.Id + " - (Width: " + grid.X +
                                                  ") - Type: " + grid.Container.Type + " Weight:" + grid.Container.Weight);
                            }
                        }
                    }
                }
